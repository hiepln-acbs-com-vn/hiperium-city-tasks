...
export class DaysPipe implements PipeTransform {
  ...
  transform(daysString: string, endIndex: number = 0): string {
    if (!daysString || daysString.length === 0) {
      return '';
    }
    const daysList: string[] = daysString.split(',');
    const newDaysList = daysList.map(day => this.getDayName(day, endIndex));
